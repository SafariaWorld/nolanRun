...
class PlayScene extends Phaser.Scene {
    ...
    createHorethBall() {
        ...
        if (this.currentHorethBallNumber < this.maxHorethBall) {
            this.horethBall = this.physics.add.sprite(this.player.x, this.player.y, 'horethBall');
            this.horethBall.setScale(0.5);
            this.playerDamageGroup = this.horethBall;
    ...
    createSnakeCollider() {
        this.physics.add.collider(this.enemyGroup, this.horethBall, function() {
            this.physics.pause();
        });
    }

    moveSnake() {
        this.snake.setVelocityX(-300);
